...
def qaoa_circuit(params, noise_param):
    ...
    for gamma, alpha in params:
        # Apply cost layer, noting that in the case of the ZZ terms, they can be decomposed as CNOT-RZ-CNOT
        for w in range(num_wires):
            qml.RZ(2 * gamma * coeffs[w], w)
        for edge, coeff in zip(edges, coeffs[4:]):
        ...
        for i, (control_qubit, target_qubit) in enumerate(edges):
            qml.RX(2 * alpha, wires=target_qubit)
...
def approximation_ratio(qaoa_depth, noise_param):
    ...
    for s in range(steps):
        params, prev_val = opt.step_and_cost(
            lambda p: qaoa_circuit(p, noise_param), params
        )
        if s % 20 == 0:
            print(f"Step {s}: cost = {prev_val}")
...
circuit_check = np.isclose(
    noiseless_qaoa(random_params) - qaoa_circuit(random_params, 0), 0
)
...
def check(solution_output: str, expected_output: str) -> None:
    ...
    names = list(qml.specs(qaoa_circuit)([[1, 1]], 0.1)["resources"].gate_types.keys())
    random_params = np.array([np.random.rand(2)])
    ...
    assert set(names) == {
        ...
        "RZ",
        "CNOT",
    }, "Your circuit must use qml.RX, qml.RY, qml.RZ, qml.CNOT, and qml.DepolarizingChannel."

    assert solution_output > expected_output - 0.02


# These are the public test cases
test_cases = [("[2,0.005]", "0.4875"), ("[1, 0.003]", "0.1307")]
