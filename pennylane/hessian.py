...
def compute_hessian(num_wires, w):
    ...
    @qml.qnode(dev)
    def circ(w):
        x=[i for i in w[:num_wires]]
        qml.AngleEmbedding(features=x,wires=range(num_wires),rotation='X')
        for i in range(num_wires):
            qml.CNOT(wires=[i,(i+1)%num_wires])
        qml.RY(w[num_wires],1)
        for i in range(num_wires):
            qml.CNOT(wires=[i,(i+1)%num_wires])
        qml.RX(w[num_wires+1],num_wires-1)
        return qml.expval(qml.Z(0)@qml.Z(num_wires-1))
    # Return the Hessian using qml.gradient.param_shift_hessian
    return  qml.gradients.param_shift_hessian(circ)(w)
